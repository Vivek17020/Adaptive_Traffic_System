...
speeds = {'car':2.25, 'bus':1.8, 'truck':8.2, 'rickshaw':2, 'bike':2.5,'ambulance':2.5}  # average speeds of vehicles
...
#       greenTime = defaultMaximum
    ...
    truckProbability = random.randint(0, 9)  # Adjust this probability as needed
    if truckProbability <= 2:
        noOfTrucks += 1 
    ...
    # Set red time for all other signals
    for i in range(noOfSignals):
        if i != currentGreen:
            signals[i].red = defaultRed + defaultYellow + signals[currentGreen].green
...
# Global variables to track time for truck generation
last_truck_generated_time = time.time()
truck_generation_interval = 60 
