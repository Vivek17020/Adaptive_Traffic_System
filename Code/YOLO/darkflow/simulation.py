...
speeds = {'car':2.25, 'bus':1.8, 'truck':5.8, 'rickshaw':2, 'bike':2.5,'ambulance':2.5}  # average speeds of vehicles
...
def setTime():
    ...
    noOfCars, noOfBikes, noOfBuses, noOfTrucks, noOfRickshaws, noOfAmbulance = 0, 0, 0, 0, 0, 0
    ...
    # Adjust the green time based on the presence of trucks
    total_vehicle_count = noOfCars + noOfBuses + noOfTrucks + noOfRickshaws + noOfBikes + noOfAmbulance
    ...
    if noOfTrucks > 0:
        # If there are trucks, set the green time to a higher value
        greenTime = defaultMaximum  # Or any value you want for trucks
    
    # Ensure the green time is within the limits
...
def generateVehicles():
    while True:
        ...
        # Adjust the probability of generating trucks
        if vehicle_type == 2:
            # Reduce the probability of generating trucks by increasing the threshold
            if random.random() < 0.9:  # Adjust this threshold as needed
                continue  # Skip generating the truck in this iteration
